# -*- coding: utf-8 -*-
"""Thesis_CodeWorking.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1H1eMCcoQoc4l8jyoukg2ny9tGHZYK2cE
"""
...
austin_boundary = boroughs  # Assuming this file represents Austin boundary
...
# Ensure CRS matches for distance calculations
...
# Ensure the selected columns exist in the Austin listings data
...
# Assuming the original listings dataframe contains the 'neighbourhood_cleansed' column
...
# Adjust code to ensure 'neighbourhood_cleansed' is included in 'mean'
mean = (
    listings_austin_subset.groupby("neighbourhood_cleansed")["ols_m_r"]
    .mean()
    .reset_index()  # This will add 'neighbourhood_cleansed' as a column
...
for idx, row in austin_n_r.iterrows():
    ...
    if zip_code:  # Only add annotation if zip code is available
        plt.annotate(
            text=zip_code,
...
# Ensure 'geometry' column is included from 'listings_austin' data
...
# Make a copy of your listings_austin data for sampling if needed (optional step)
listings_austin_sample = listings_austin.copy()

# Ensure that the dependent variable 'log_price' and explanatory variables are defined
...
# Option B: Limit the maximum iterations
# selector = Sel_BW(coords, y, exp_vars, multi=True, spherical=True)
# optimal_bandwidth = selector.search(multi_bw_min=[4], max_iters=50)  # Limit iterations to 50
...
# Add 'distance_to_attraction' and 'log_price' for spatial and pricing influence
clustering_vars = listings_austin[['latitude', 'longitude', 'log_price',
                                   'distance_to_attraction',
                                   'room_type_Entire home/apt',
                                   'room_type_Private room',
                                   'room_type_Shared room',
                                   'room_type_Hotel room']].copy()

# Step 2: Standardize the Data
scaler = StandardScaler()
clustering_vars_scaled = scaler.fit_transform(clustering_vars)

# Step 3: Elbow Method to Determine Optimal Clusters
inertia = []
for k in range(1, 10):
    kmeans = KMeans(n_clusters=k, random_state=0).fit(clustering_vars_scaled)
    inertia.append(kmeans.inertia_)
...
optimal_k = 4  # Replace with the optimal value found from the elbow method
kmeans = KMeans(n_clusters=optimal_k, random_state=0)
clusters = kmeans.fit_predict(clustering_vars_scaled)

# Assign Clusters to the DataFrame
listings_austin['cluster'] = clusters

# Step 5: Visualize Clusters on Map
fig, ax = plt.subplots(figsize=(12, 10))
listings_austin.plot(ax=ax, column='cluster', cmap='viridis', markersize=5, legend=True)
ctx.add_basemap(ax, crs=listings_austin.crs, source=ctx.providers.OpenStreetMap.Mapnik)
plt.title('K-means Clustering of Airbnb Listings with Distance to Attractions')
plt.show()

# Cluster Summary
cluster_summary = listings_austin.groupby('cluster')[['latitude', 'longitude', 'log_price',
                                                      'distance_to_attraction']].mean()
cluster_summary['count'] = listings_austin['cluster'].value_counts().sort_index()
print(cluster_summary)
...
# Calculate mean values of each feature within each cluster
cluster_summary = listings_austin.groupby('cluster')[['accommodates', 'bedrooms', 'beds', 'log_price']].mean()
print(cluster_summary)

import joblib

# Save the trained GWR model to a file
joblib.dump(gwr_results, '/content/gwr_model.pkl')
print("GWR model saved successfully!")

# Save the MGWR model
joblib.dump(mgwr_results, '/content/mgwr_model.pkl')
print("MGWR model saved successfully!")

from google.colab import files

# Download the GWR model
files.download('/content/gwr_model.pkl')

# Download the MGWR model
files.download('/content/mgwr_model.pkl')

# Download the KMeans model (if applicable)
...
# Load the model
gwr_model = joblib.load('gwr_model.pkl')  # Adjust the path if needed
...
# Re-save the GWR model
joblib.dump(gwr_model, 'gwr_model_compatible.pkl')
print("Model re-saved successfully!")

from google.colab import files

# Download the re-saved model
files.download('gwr_model_compatible.pkl')
